...
```python
# 4. Write a python programme to find the maximum frequency of a character in a given string.
...
```python
# 5. write a python program to add "ing" at the end of  given string (length to should be atlest 3). If the givne stirng already ends with "ing" then add "ly" insteaed . if the stirng length of the given string is less than 3, leave it unchanged.
...
```python
# 6. WAP to input integer values in a list and perform Linear search or Binary search. Also display the position of the number in the list.
...
```python
# 7. Write a python prgram to remove duplicates from a an integer first(Elements supplied by the user).  Find 2 largest number from the list.
...
```python
# 9. write a menu driven prgram in python which will accept a number and choices as "," from the user and calculate the following accoring the the choice
# a. Factorial of each digit
# b. reverse of that number
# c. Factors of that number and display the factors.
def factorial(n):
    if n == 0 or n == 1:
        return 1
    else:
...
while True:
    print("\nMenu:")
    print("a. Factorial of each digit")
    print("b. Reverse of the number")
    print("c. Factors of the number")
    print("q. Quit")
    ...
    if choice == 'a':
        ...
        digit_factorials = [factorial(digit) for digit in digits]
        print("Factorial of each digit:", digit_factorials)
        ...
        print("Factors of the number:", num_factors)
...
```python
# 10.Write a prgram in python to input the lower and upper limit and print all the prime number lies between the ranges . Also print the following informatio
